const searchSelect =  document.querySelector("#search_expenses");
const expenses_row =  document.querySelector("#expenses-rows");
          ...
          if (data.length > 0) {
            ...
            data.forEach(el => {
                ...
                create_row.innerHTML = `
                    ...
                    <td>${el.category}</td>
                    <td>${el.date}</td>
                    <td><a href="${location.href}edit-expenses/${el.id}" class="btn btn-warning btn-sm">Edit</a></td>
                `
